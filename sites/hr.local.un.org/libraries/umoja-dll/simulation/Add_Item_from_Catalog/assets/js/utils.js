...
Utils.window = {
	...
	isSameDomain: function(winRefStr)
	{
		...
		try
		{
			...
			{
				if (winRefStr == 'top') {
					Utils.debug.trace('Window name is '+top.window.name);
					winRef.name = top.window.name;
				} else if (winRefStr == 'parent'){
					Utils.debug.trace('Window name is '+parent.window.name);
					winRef.name = parent.window.name;
				} else {
					Utils.debug.trace('Window name is '+winRef.name);
	...
	createTextField: function(o)
	{
		...
		try{
			...
			if (curFontSize.match(numRegex))
			{
				...
				//alert('oFont:' + curFontSize + ' newFont: '+fontSize + ' oHeight: '+ o.h + ' spanHeight: '+lHeight);
