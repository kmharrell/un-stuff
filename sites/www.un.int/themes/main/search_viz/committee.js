...
(function($){
          ...
          moved:function(e){
            ...
            if (nearest.node.data.shape!='dot'){
              ...
              if (selected){
                ...
                if (selected.node.data.link == undefined) {
            ...
            }else if (nearest.node.data.type == 'topLevelNode') {
  ...
  $(document).ready(function(){
    ...
    var promise = $.getJSON("http://pm:8888/committee-json", function(data){
      ...
      var nodes = {};
      var edges = {};
      $.each(importNodes, function(name, info){
        //console.log(info.node);
        var parentKey = info.node.Parent;
        var key = info.node["name"];
        nodes[key] = {
          color: "#0a598b",
          alpha: 0,
          link: "/search?f[0]=field_un_themes%3A"+info.node['Term ID'],
        };
        if (edges[parentKey] == undefined) {
          edges[parentKey] = {};
        }
        if (info.node.Parent == "" || info.node.Parent == undefined) {
          // This is a parent-node
          parentKey = 'Committee';
          nodes[key].type = 'topLevelNode';
          nodes[key].shape = 'dot';
          nodes[key].alpha = 1;
          nodes[key].color = "#56b3da";
          if (edges[parentKey] == undefined) {
            nodes[parentKey] = { color: "#0598ff", shape: "dot", alpha: 1, link: "/search" };
            edges[parentKey] = {};
          }
          edges[parentKey][key] = {length: 8};
        }
        else {
          edges[parentKey][key] = {};
        }
      ...
      theUI = {nodes:nodes, edges:edges};
      var sys = arbor.ParticleSystem()
      sys.parameters({stiffness:900, repulsion:2000, gravity:true, dt:0.015})
      sys.renderer = Renderer("#viewport-committee")
      sys.graft(theUI)
    ...
    /*
    var theUI = {
      nodes:{"Root Topic":{color:"red", shape:"dot", alpha:1},

        parent:{type: 'toplev', color:CLR.branch, shape:"dot", alpha:1},
        ...
        docs:{type: 'toplev', color:CLR.branch, shape:"dot", alpha:1},
        reference:{color:CLR.doc, alpha:0, link:'#reference'},
        introduction:{color:CLR.doc, alpha:0, link:'#introduction'},

        code:{type: 'toplev', color:CLR.branch, shape:"dot", alpha:1},
